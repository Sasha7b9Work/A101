...
static pchar GetKA()
{
    return set.lang == Lang::RU ? "кА" : "kA";
}

static pchar GetMA()
{
    return set.lang == Lang::RU ? "мА" : "mA";
}

static pchar GetUA()
{
    return set.lang == Lang::RU ? "мкА" : "uA";
}


static pchar GetNA()
{
    return set.lang == Lang::RU ? "нА" : "nA";
}
...
void DiagramInput::DrawTimeScale()
{
    ...
    sprintf(buffer, "%.0f %s", time_ms, (set.lang == Lang::RU) ? "мс" : "ms");
