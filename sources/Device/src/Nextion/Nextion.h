...
namespace Nextion
{
    // ‘ункци€ вызываетс€ в главном цикле
    void Update();

    bool ExixtCommnadsForExecute();

    // Ёта функци€ вызваетс€ по приходу байта с диспле€
    ...
    void DrawLineWhite(int x1, int y1, int x2, int y2);
    ...
    static bool exist_command = false;      // ≈сли true - есть команды дл€ исполнени€
